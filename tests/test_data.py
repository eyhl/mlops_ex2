...
def test_processed_data():
    ...
    assert len(train_data) == 40000 and len(test_data) == 5000

    # check that all images has shape [1, 28, 28]
    assert all([train_data[i][0].shape == torch.Size([1, 28, 28]) for i in range(len(train_data))])
    assert all([test_data[i][0].shape == torch.Size([1, 28, 28]) for i in range(len(test_data))])

    # check that all labels are present 0..9
    assert all(torch.unique(train_data[:][1]) == torch.arange(10))
    assert all(torch.unique(test_data[:][1]) == torch.arange(10))